<script lang=ts>
	...
	onMount(() => {
		...
		socket.on("gameState", (data) => {
			player.y = height * data.player.y;
			enemy.y = height * data.opponent.y;
			ball.x = width * data.ball.x;
			ball.y = height * data.ball.y;
		});
